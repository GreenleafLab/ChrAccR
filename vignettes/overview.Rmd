...
The example data used in this vignette was designed for illustrating the functionality of `ChrAccR` rather than a full analysis. It contains ATAC-seq data for a subset of different T cell samples from the immune atlas dataset [@Calderon:2018ha] and it focuses on only a subset of the human genome (approximately 1%). It can be loaded using:
```{r, message = FALSE, warning = FALSE}
...
devZ <- chromVAR::deviations(cvRes)
str(devZ)
...
# motifNames <- grep("(IRF8|CTCF|GATA3)$", names(prepareMotifmatchr("hg38", "jaspar")$motifs), value=TRUE) # alternative by searching for patterns
...
str(fps[["MA0652.1_IRF8"]]$footprintDf)
# the result also contains a ggplot object, which can directly be plotted
