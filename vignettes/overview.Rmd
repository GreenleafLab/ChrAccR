...
The example data used in this vignette was designed for illustrating the functionality of `ChrAccR` rather than a full analysis. It contains ATAC-seq data for a subset of different T cell samples from the immune atlas dataset [@Calderon:2019cb] and it focuses on only a subset of the human genome (approximately 1%). It can be loaded using:
```{r, message = FALSE, warning = FALSE}
